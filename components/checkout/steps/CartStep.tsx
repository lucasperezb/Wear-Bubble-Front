    ...
    <section>
      ...
      <div className="border border-bubble-line bg-bubble-white px-6">
        {lines.map(({ item, product }) => {
          ...
          return (
            <div
              className="grid grid-cols-[96px_minmax(0,1fr)_auto] gap-5 border-b border-bubble-line py-5 last:border-b-0 max-[620px]:grid-cols-[72px_1fr]"
              key={`${item.pid}-${item.color || "legacy"}-${item.size}-${item.bundle || "single"}`}
            >
              ...
              <div className="flex min-w-0 flex-col">
                {item.bundle ? (
                  <span className="mb-1 font-sans text-[.64rem] font-bold uppercase tracking-[.08em] text-bubble-success">
                    Conjunto · 5% OFF
                  </span>
                ) : null}
                {promo ? (
                  <span className="mb-1 font-sans text-[.62rem] font-bold uppercase tracking-[.08em] text-bubble-danger">
                    Promoção · {promotionPct(product)}% OFF
                  </span>
                ) : null}
                ...
                <div className="mt-auto flex items-center gap-3 pt-3">
                  <button
                    className="size-8 rounded-full border border-bubble-ink bg-transparent"
                  ...
                  <button
                    className="ml-2 border-0 bg-transparent font-sans text-[.68rem] font-semibold uppercase text-bubble-danger underline"
                    onClick={() => onQty(item, -item.qty)}
                  >
                    Remover
                  </button>
                </div>
              </div>
              <strong className="text-right max-[620px]:col-span-2">
                {money.format(finalPrice * item.qty)}
              </strong>
            </div>
          );
        })}
      ...
      <div className="mt-6 max-w-[420px]">
        ...
        {coupon ? (
          ...
          <div className="flex border border-bubble-line bg-bubble-white p-1">
            ...
            />
            <button
              className="bg-bubble-ink px-5 font-sans text-[.7rem] font-semibold uppercase text-bubble-white"
