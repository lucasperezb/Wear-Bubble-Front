...
export function ConfirmationStep({ order, orderNumber, email }: { order: Order | null; orderNumber?: string; email?: string }) {
  const loginHref = `/login?modo=codigo${email ? `&email=${encodeURIComponent(email)}` : ''}`;
  return (
    <section className="border border-bubble-line bg-bubble-white p-8 text-center">
      ...
      {order ? (
        <>
          ...
          <p className="mt-2 text-[.78rem] text-bubble-ink/55">{order.status === 'paid' ? 'Pagamento confirmado.' : 'Aguardando a confirmacao do pagamento.'}</p>
        </>
      ) : (
        <>
          <p className="mt-3 text-bubble-ink/65">{orderNumber ? <>Pedido <b>#{orderNumber}</b>.</> : 'Seu pedido foi criado.'} Sera atualizado assim que o pagamento for confirmado.</p>
          <p className="mt-2 text-[.78rem] text-bubble-ink/55">Para acompanhar a compra, entre usando o codigo enviado ao seu e-mail.</p>
        </>
      )}
