...
export function ProductCard({ product, href }: ProductCardProps) {
  ...
  function move(event: React.MouseEvent<HTMLElement>) {
    if (!visualRef.current || !mediaRef.current || !hasFinePointer()) return;
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-1 flex-col gap-2 px-5 pb-5 pt-[22px] max-[520px]:gap-1.5 max-[520px]:px-3 max-[520px]:pb-3 max-[520px]:pt-3.5">
        <div className="text-[.7rem] uppercase tracking-[.1em] text-bubble-ink/50 max-[520px]:truncate max-[520px]:text-[.66rem]">{product.sub}<span className="max-[520px]:hidden"> · <span className="text-[.62rem] text-bubble-ink/45">{product.material}</span></span></div>
