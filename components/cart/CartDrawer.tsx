...
type CartDrawerProps = {
  ...
  cart: CartItem[];
  products: Product[];
  onQty: (pid: number, size: string, bundle: string | null | undefined, delta: number) => void;
  onClose: () => void;
};
            ...
            <>
              <div className="mb-[18px]">
                <div className={`mb-2 text-[.74rem] ${freeShippingRemaining <= 0 ? 'font-semibold text-bubble-success' : 'text-bubble-ink/70 [&_b]:text-bubble-brown'}`}>
                  {freeShippingRemaining <= 0 ? <>Voce ganhou <b>FRETE GRATIS</b>!</> : <>Faltam <b>R$ {freeShippingRemaining.toFixed(2).replace('.', ',')}</b> para o frete gratis</>}
                </div>
              ...
              {lines.map(({ item, product }) => (
                <div className="flex gap-3.5 border-b border-bubble-line py-4" key={`${item.pid}-${item.size}-${item.bundle || 'single'}`}>
                  <div className="flex h-[74px] w-[62px] shrink-0 items-center justify-center bg-bubble-cream2 [&_svg]:w-3/5"><ProductIcon icon={product.icon} /></div>
                  <div className="flex flex-1 flex-col gap-[3px]">
                    ...
                    <div className="text-[.7rem] text-bubble-ink/50">Tam. {item.size} · R$ {product.price.toFixed(2).replace('.', ',')} un.</div>
                    <div className="mt-1.5 flex items-center gap-2.5 [&_button]:size-[22px] [&_button]:border [&_button]:border-bubble-line [&_button]:bg-transparent [&_button]:text-[.8rem]">
                      <button onClick={() => onQty(item.pid, item.size, item.bundle, -1)}>−</button><span>{item.qty}</span><button onClick={() => onQty(item.pid, item.size, item.bundle, 1)}>+</button>
                    </div>
                    <button className="mt-[5px] self-start border-0 bg-transparent text-[.68rem] text-bubble-brown underline" onClick={() => onQty(item.pid, item.size, item.bundle, -item.qty)}>Remover</button>
                  </div>
                </div>
              ))}
            </>
